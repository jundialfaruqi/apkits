                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="mb-3">
                                                ...
                                                <input type="text"
                                                    class="form-control rounded-4 @error('name') is-invalid
                                                            @enderror"
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="mb-3">
                                                ...
                                                <input type="text"
                                                    class="form-control rounded-4 @error('email') is-invalid
                                                            @enderror"
                                                    name="email" placeholder="your@mail.com" value="{{ old('email') }}">
                                        ...
                                        <div class="col-md-12">
                                            <div class="mb-3">
                                                ...
                                                <select id="opd_id" name="opd_id"
                                                    ...
                                                    <option value="" class="dropdown-header" disabled selected>Pilih
                                                        OPD</option>
                                                    @foreach ($opds as $opd)
                                                        <option value="{{ $opd->id }}"
                                        ...
                                        <div class="col-md-12">
                                            <div class="mb-2">
                                                ...
                                                <input type="password" id="password"
                                                    class="form-control rounded-4 @error('password') is-invalid 
                                                        @enderror"
        ...
        $(document).ready(function() {
            ...
            function updateBidangDropdowns(opdId) {
                ...
                bidangSelect.empty().append(
                    '<option value="" class="dropdown-header" disabled selected>Pilih Bidang</option>');
                pekerjaanSelect.empty().append(
                    '<option value="" class="dropdown-header" disabled selected>Pilih Pekerjaan</option>');
                ...
                if (selectedOpd && selectedOpd.formatlaporans && selectedOpd.formatlaporans.length > 0) {
                    ...
                    uniqueBidang.forEach(bidang => {
                        if (bidang) {
                            bidangSelect.append(`<option value="${bidang}">${bidang}</option>`);
                        }
                    });

                    // Populate pekerjaan dropdown
                    Object.entries(pekerjaans).forEach(([id, nama_pekerjaan]) => {
                        pekerjaanSelect.append(`<option value="${id}">${nama_pekerjaan}</option>`);
                    });
                }
            }

            // If not super admin and only one OPD, auto-select it and update other dropdowns
            if (!isSuperAdmin && opds.length === 1) {
                $('#opd_id').val(opds[0].id);
                updateBidangDropdowns(opds[0].id);
            }
            ...
            // Trigger change event on page load if an OPD is already selected
